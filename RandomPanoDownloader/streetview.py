...
def download_panorama_v3(panoid, zoom=5, disp=False, filename=None,alternate=False):
    '''
    ...
    # if alternate:
        # 416*(2**(zoom-2)), 416*( 2**(zoom-3) )
    tiles = tiles_info( panoid, zoom=zoom, alternate=alternate)
    valid_tiles = []
    ...
    for i, tile in enumerate(tiles):
        ...
        if x*tile_width < img_w and y*tile_height < img_h: # tile is valid
            ...
            try:
                ...
                if disp: print(response.content,url)
                return False
